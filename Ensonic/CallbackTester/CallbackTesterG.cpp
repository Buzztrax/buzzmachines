...
#ifdef _DEBUG
  // test api hacks
  sprintf(DebugStr,"  GetEnvSize(-1,-1): %d",pCB->GetEnvSize(-1, -1));
  OutputDebugString(DebugStr);
  sprintf(DebugStr,"  GetEnvSize(-1,-1): %d",pCB->GetEnvSize(-1, -1));
  OutputDebugString(DebugStr);
  sprintf(DebugStr,"  GetEnvSize(-1,-2): %d",pCB->GetEnvSize(-1, -2));
  OutputDebugString(DebugStr);
  sprintf(DebugStr,"  GetEnvSize(-2,-1): %d",pCB->GetEnvSize(-2, -1));
  OutputDebugString(DebugStr);
  sprintf(DebugStr,"  GetEnvSize(-3,-1): %d",pCB->GetEnvSize(-3, -1));
  OutputDebugString(DebugStr);


  // test that attributes and parameters are initialized
...
void mi::Tick()
{
  ...
  OutputDebugString(DebugStr);
    sprintf(DebugStr,"    tval[0].attack=%d",tval[0].attack);
  OutputDebugString(DebugStr);
    // shouldn't this be initialy 0? (it is not), [0..SamplesPerTick-1]
    sprintf(DebugStr,"  mi::Tick(%d) < %d",pMasterInfo->PosInTick, pMasterInfo->SamplesPerTick);
  OutputDebugString(DebugStr);
#endif
