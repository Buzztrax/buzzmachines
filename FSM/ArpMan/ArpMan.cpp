...
void mi::SetFilter_4PoleLP(CChannel &c, float CurCutoff, float Resonance)
{
  ...
  float CutoffFreq=(float)(264*pow(32,CurCutoff/240.0));
	...
	if (cf>=(sr/2.1)) cf=sr/2.1; // pr�ba wprowadzenia nieliniowo�ci przy ko�cu charakterystyki
  ...
  float a0=float(1/(1+A+B));
	c.Biquad.m_b1=2*(c.Biquad.m_b2=c.Biquad.m_b0=a0);// obliczenie wsp�czynnik�w filtru cyfrowego (przekszta�cenie dwuliniowe)
	...
	B=fB*ncf*ncf;
  a0=float(1/(1+A+B));
	c.Biquad2.m_b1=float(2*(c.Biquad2.m_b2=c.Biquad2.m_b0=0.35f*a0/fQ));// obliczenie wsp�czynnik�w filtru cyfrowego (przekszta�cenie dwuliniowe)
